...
uint32_t %s(void* p) {
	...
	def __arg_from_rust_to_c(self, val, arg_name):
		...
		# special Slice
			...
			c_call += convert_rust_to_c({"conv": val["conv"].T_conv}, f"{slice_name}ToC.Data", f"{slice_name}ToCBuf", 1)
		# std function
		elif "RustStdFunctionConverter" in str(val["conv"]):
		...
		for type in self._bound_types:
			...
			if type.is_type_class():
				...
				for member in type.members:
					rust_bind += f"pub fn {clean_name_with_title(self._name)}{clean_name_with_title(type.bound_name)}Get{clean_name_with_title(member['name'].naked_name())}({clean_name_with_title(type.bound_name).lower()}: *mut {clean_name_with_title(type.bound_name)}) -> {self.select_ctype_conv(member['ctype'])};\n"
					rust_bind += f"pub fn {clean_name_with_title(self._name)}{clean_name_with_title(type.bound_name)}Set{clean_name_with_title(member['name'].naked_name())}({clean_name_with_title(type.bound_name).lower()}: *mut {clean_name_with_title(type.bound_name)},value : {self.select_ctype_conv(member['ctype'])});\n"
	...
	def finalize(self):
		...
		self.rust_bind += self._write_rust_header()
